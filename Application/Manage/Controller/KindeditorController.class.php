...
class KindeditorController extends Controller {
    ...
    function thumb_img($img_path, $thumb_w, $save_path, $is_del = true) {
        ...
        if ($width > $thumb_w) {//如果宽度大于指定宽度进行操作
    ...
    function upload_phone() {
        //文件保存目录路径
        $save_path = THINK_PATH . '../uploadfile/image/';
        //文件保存目录URL
        $save_url = C("WEB_URL") . '/uploadfile/image/';
        $return = "";
        ...
        if (preg_match('/^(data:\s*image\/(\w+);base64,)/', $base64_image_content, $result)) {
            ...
            __mkdirs($filepath);
            $filename_ = date("YmdHis") . '_' . rand(10000, 99999);
            $filename = $filename_ . '.' . $file_ext;
            if (file_put_contents($filepath . $filename, base64_decode(str_replace($result[1], '', $base64_image_content)))) {
                //$return=$filename;
                //缩放图片
                $return = thumb_img($filepath . $filename, 800, $filepath . $filename_ . '_800.' . $file_ext, false);
            } else {
                $return = "";
            }
        }

        $return = $save_url . date("Ymd") . '/' . $filename;
        echo $return;
        exit();
    }
    ...
    public function file_manager_json() {
        ...
        $root_path = THINK_PATH . '../uploadfile/';
        //根目录URL，可以指定绝对路径，比如 http://www.yoursite.com/attached/
        $root_url = C("WEB_URL") . '/uploadfile/';
        //图片扩展名
